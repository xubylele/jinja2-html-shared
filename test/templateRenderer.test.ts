...
describe("templateRenderer", () => {
  describe("renderTemplate", () => {
    ...
    it("processes for loops", () => {
      const result = renderTemplate("{% for item in items %}{{ item }} {% endfor %}", {
        items: ["a", "b"],
      });
    ...
    it("processes if conditions", () => {
      const result = renderTemplate("{% if show %}yes{% else %}no{% endif %}", { show: true });
  ...
  describe("findMissingVariables", () => {
    ...
    it("does not flag for-loop variables as missing", () => {
      const missing = findMissingVariables("{% for i in items %}{{ i }}{% endfor %}", {
        items: [1, 2],
      });
