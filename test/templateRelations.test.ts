import { scanTemplateRelations, extractMacroDefinitions } from "../src/templateRelations";
...
describe("scanTemplateRelations — imports", () => {
  ...
  it('parses {% from "x" import a, b as c %}', () => {
    ...
    expect(imp.path).toBe("macros.html");
    expect(imp.names).toEqual([{ name: "card" }, { name: "modal", alias: "m" }]);
  });
...
describe("scanTemplateRelations — combined", () => {
  it("collects everything in a realistic child template", () => {
    ...
    expect(r.imports.map((i) => i.path).sort()).toEqual(["macros/cards.html", "macros/forms.html"]);
